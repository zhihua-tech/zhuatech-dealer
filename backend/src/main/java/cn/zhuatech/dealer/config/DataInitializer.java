...
package cn.zhuatech.dealer.config;
import cn.zhuatech.dealer.domain.DomainCatalog;
import cn.zhuatech.dealer.model.*;
import cn.zhuatech.dealer.repository.*;
...
@Configuration public class DataInitializer {
 ...
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "DMS-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="DMS-DEMO-"+String.format("%03d",sequence);
